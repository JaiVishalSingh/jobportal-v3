  ...
  const options = data.filter((item) => item.toLowerCase().includes(search.trim().toLowerCase())).map((item) => (
    <Combobox.Option value={item} key={item} active={value.includes(item)}>
      <Group gap="sm">
        ...
        />
        <span className='text-cape-cod-300'>{item}</span>
      </Group>
    </Combobox.Option>
  ));
    ...
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        <PillsInput variant='unstyled' rightSection={<IconSelector />} onClick={() => combobox.toggleDropdown()}
          leftSection={
            <div className='text-blue-400 p-1 bg-cape-cod-900 rounded-full mr-2'><props.icon /></div>
          }>
          <Pill.Group>
            {value.length > 0 ? (
              <>
              ...
              <Input.Placeholder className='text-cape-cod-200'>{props.title}</Input.Placeholder>
            )}
